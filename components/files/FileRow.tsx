...
import { Folder, File as FileIcon, Download, Edit2, Trash2, Check, X, FileCode, FileJson, FileType, Image, FileText, FileArchive, FileCog } from 'lucide-react';
...
export const FileRow: React.FC<FileRowProps> = ({ 
    ...
    const getFileIcon = (fileName: string) => {
        ...
        switch(ext) {
            case 'js':
            case 'jsx':
                return <FileCode className="w-5 h-5 text-yellow-500 shrink-0" />;
            case 'ts':
            case 'tsx':
            ...
            case 'scss':
            case 'less':
                return <FileType className="w-5 h-5 text-sky-400 shrink-0" />;
            case 'html':
            case 'htm':
            case 'php':
                return <FileCode className="w-5 h-5 text-orange-500 shrink-0" />;
            case 'json':
                return <FileJson className="w-5 h-5 text-neutral-500 shrink-0" />;
            case 'env':
            case 'config':
            case 'lock':
                return <FileCog className="w-5 h-5 text-slate-500 shrink-0" />;
            case 'md':
            case 'txt':
            ...
            default:
                return <FileIcon className="w-5 h-5 text-slate-300 shrink-0" />;
        }
    };
        ...
        >
            <div className="col-span-6 flex items-center gap-3 text-slate-700 group-hover:text-indigo-700 transition-colors">
                ...
                {renamingId === file.id ? (
                    ...
                    <span className={`font-medium truncate ${file.type === 'folder' ? 'text-slate-800' : 'text-slate-600'}`}>{file.name}</span>
                )}
            </div>
