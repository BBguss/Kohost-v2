...
import { useDeployment, DeployStage, DbMode } from '../../contexts/DeploymentContext';
...
export const CreateSite: React.FC<CreateSiteProps> = ({ domains, onDeploy, user, sites = [], plans = [], onUpgrade }) => {
  ...
  useEffect(() => {
      if (deployStage !== 'EXTRACTING' || !user) return;
      ...
      socket.on(`deploy_progress_${user.id}`, (data: { stage: string, progress: number }) => {
  ...
  const simulateUpload = async () => {
      // Faster upload simulation since the real upload happens during 'EXTRACTING' due to API structure
      // In a real scenario, this would be hooked to Axios onUploadProgress
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        ...
        // This stage includes both Uploading bytes to server AND Extraction
        setDeployStage('EXTRACTING');
        setExtractProgress(0); // Reset for real data
        
        // The API call will block until extraction is done on server
        // Progress will be updated via socket effect above
  ...
  const steps = [
      { id: 'UPLOADING', label: 'Preparing Upload', icon: Upload },
      { id: 'EXTRACTING', label: 'Uploading & Extracting', icon: FileArchive },
  ...
  const getExtractionStatusMessage = (progress: number) => {
      if (progress < 5) return "Starting extraction engine...";
      if (progress < 30) return "Analyzing archive structure...";
      if (progress < 60) return "Unzipping project files...";
      if (progress < 90) return "Writing files to disk...";
      return "Finalizing file permissions...";
  };
                              ...
                              <div className="space-y-2 mb-6 animate-in slide-in-from-bottom-2">
                                  <div className="flex justify-between text-xs font-semibold text-slate-600">
                                      <span className="flex items-center gap-1">
                                          <FileCog className="w-3 h-3 animate-bounce" /> 
                                          {getExtractionStatusMessage(extractProgress)}
                                      </span>
                                      <span>{Math.floor(extractProgress)}%</span>
                                  </div>
                                  <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden relative">
                                      <div 
                                        className={`h-full rounded-full transition-all duration-300 ease-out relative overflow-hidden ${extractProgress > 90 ? 'bg-amber-500' : 'bg-indigo-600'}`}
                                  ...
                                  <p className="text-[10px] text-slate-400 text-center pt-1">
                                      {extractProgress > 85 ? "This might take a moment for large projects with many files." : "Unpacking archive on server..."}
                                  </p>
                              </div>
                          )}
