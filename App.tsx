...
import { VerifyEmailPage } from './pages/VerifyEmailPage'; // Email Verification Page
...
const App: React.FC = () => {
  ...
  // Email Verification State - untuk menangani user yang belum verifikasi
  const [verificationNeeded, setVerificationNeeded] = useState<{
    show: boolean;
    email: string;
    userId: string;
  }>({ show: false, email: '', userId: '' });
  const [resendingEmail, setResendingEmail] = useState(false);

  // Email Verification Page State - untuk deteksi URL /verify-email
  ...
  // URL Detection untuk Email Verification
  // Cek apakah URL saat ini adalah /verify-email?token=xxx
  useEffect(() => {
    ...
    if (pathname === '/verify-email' && token) {
      console.log('[App] Detected verify-email URL with token');
  ...
  // Initial Load
  useEffect(() => {
    // Skip init jika sedang di halaman verify email
    if (verifyEmailToken) {
      setLoading(false);
      return;
    }
  ...
  const handleAuthSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setAuthError('');
    setVerificationNeeded({ show: false, email: '', userId: '' });
    setAuthLoading(true);
    ...
    try {
      if (isRegistering) {
        ...
        // Tampilkan pesan untuk cek email, bukan langsung suruh login
      ...
      // Cek apakah error adalah EMAIL_NOT_VERIFIED
      // Error dari backend: { message, code, email, userId }
      if (e.message?.includes('verify your email') || e.code === 'EMAIL_NOT_VERIFIED') {
        // Tampilkan UI verifikasi email
        setVerificationNeeded({
          show: true,
          email: e.email || authForm.email || 'your email',
          userId: e.userId || ''
        });
      } else {
  ...
  const handleResendVerification = async () => {
    ...
    try {
      // Gunakan API module yang sudah ada
      const result = await api.auth.resendVerification(verificationNeeded.email);
  ...
  // Handler untuk kembali dari halaman verifikasi ke login
  ...
  // ============================================
  // Jika URL adalah /verify-email?token=xxx, tampilkan halaman verifikasi
  if (verifyEmailToken) {
    return (
      <VerifyEmailPage
        token={verifyEmailToken}
        onBackToLogin={handleBackFromVerification}
      />
    );
  }
        ...
        <div className="bg-white dark:bg-slate-800 p-8 rounded-2xl shadow-xl w-full max-w-md space-y-8 border border-slate-200 dark:border-slate-700">
          ...
          <form onSubmit={handleAuthSubmit} className="space-y-4">
            ...
            {verificationNeeded.show && (
              <div className="p-4 bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-700 rounded-lg animate-in fade-in slide-in-from-top-2">
                <div className="flex items-start gap-3">
                  <AlertTriangle className="w-5 h-5 text-amber-600 dark:text-amber-400 shrink-0 mt-0.5" />
                  <div className="flex-1">
                    <h4 className="font-bold text-amber-800 dark:text-amber-200 text-sm">Email Verification Required</h4>
                    <p className="text-xs text-amber-700 dark:text-amber-300 mt-1">
                      Please check your inbox at <strong>{verificationNeeded.email}</strong> and click the verification link to activate your account.
                    </p>
                    <div className="mt-3 flex gap-2">
                      <button
                        ...
                        className="px-3 py-1.5 text-xs font-medium bg-amber-600 hover:bg-amber-700 text-white rounded-lg transition-colors flex items-center gap-1.5"
                      >
                        {resendingEmail ? <Loader2 className="w-3 h-3 animate-spin" /> : <RefreshCw className="w-3 h-3" />}
                        Resend Email
                      </button>
                      <button
                        type="button"
                        onClick={() => setVerificationNeeded({ show: false, email: '', userId: '' })}
                        className="px-3 py-1.5 text-xs font-medium text-amber-700 dark:text-amber-300 hover:bg-amber-100 dark:hover:bg-amber-900/50 rounded-lg transition-colors"
                      >
                        Dismiss
                      </button>
                    </div>
